...
namespace utils
{
	...
	struct Allocator
	{
		...
		Allocator() = default;
		Allocator(_Ty* element) : element{ element } {}
		Allocator(const Allocator& alloc) = delete;
		Allocator(Allocator&& alloc) noexcept : element{ alloc.element } { alloc.element = nullptr; };
		~Allocator()
		{
			if (element)
				delete element;
		...
		Allocator& operator= (Allocator&& right) noexcept { return element = right.element, right.element = nullptr, *this; };

		template<typename _ElemTy, typename... _Args>
		requires SameOrDerived<_Ty, _ElemTy>
		static inline Allocator make(_Args&&... args) { return { new _ElemTy{ std::forward(args)... } }; }

		template<typename _ElemTy, typename... _Args>
		requires SameOrDerived<_Ty, _ElemTy>
		inline void build(_Args&&... args)
		{
			if (element)
				delete element;
			element = new _ElemTy{ std::forward(args)... };
		}

		template<typename _ElemTy>
		requires SameOrDerived<_Ty, _ElemTy>
			static inline Allocator attach(_ElemTy& elem) { Allocator alloc; return alloc.element = &elem, std::move(alloc);  }

		inline operator _Ty& () { return *element; }
		inline operator const _Ty& () const { return *element; }
	};
}
